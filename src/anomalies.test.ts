...
test("latencia: >3× p95 sigue siendo crítica con baseline; sin baseline no se evalúa", () => {
  const metrics = [m(true, 2000, 0), m(true, 300, 5)];
  assert.equal(detectAnomalies("c", "http", metrics, baseline).some((a) => a.severity === "critical" && /p95/.test(a.description)), true);
